...
def build_properties(
    ...
    if logic_name == pml_logics.GoedelFuzzyLogic().name:
        return build_godel_properties(device, scaler, feature_names, label_encoder, logic)

    if logic_name == pml_logics.DL2().name:
        return build_dl2_properties(device, scaler, feature_names, label_encoder, logic)
